#!/usr/bin/env python3
"""Create, attack, evaluate, and summarize the final CPM dataset."""
...
WORK = HERE / ".work"
CLEAN_ROOT = HERE / "seeded-simulations"
...
MBD_MAIN = REPO / "Generator/MBD_systems/main.py"
RESULTS_GENERATOR = REPO / "Generator/MBD_systems/PRV-results/generate.py"
PSEUDONYM_INTERVAL_SECONDS = 100
...
def available_memory_bytes() -> int:
    ...
    if "MemAvailable" not in values:
        raise PipelineError("Cannot determine available host memory")
    return values["MemAvailable"]


def ensure_host_capacity(manifest: dict[str, Any], stage: str) -> None:
    gib = 1024 ** 3
    safety = manifest["safety"]
    required_disk = int(safety["minimum_available_disk_gib"]) * gib
    available_disk = shutil.disk_usage(HERE).free
    if available_disk < required_disk:
        raise PipelineError(
            f"Refusing {stage}: only {available_disk / gib:.1f} GiB disk is free; "
            f"at least {required_disk / gib:.0f} GiB is required"
        )
    memory_key = f"{stage}_minimum_available_memory_gib"
    required_memory = int(safety[memory_key]) * gib
    available_memory = available_memory_bytes()
    if available_memory < required_memory:
        raise PipelineError(
            f"Refusing {stage}: only {available_memory / gib:.1f} GiB RAM is available; "
            f"at least {required_memory / gib:.0f} GiB is required"
        )
...
def prepare_scenario(
    ...
    etsi = {
        ...
        "headingChange": 4,
        "velocityChange": 0.5,
        "pseudonymInterval": f"{int(manifest['pseudonym_interval_seconds'])}s",
...
def validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    if set(manifest["active_attack_seeds"]) - set(manifest["attack_seeds"]):
        raise PipelineError("An active attack seed is not defined")
    ...
    expected_windows = {
        "InTAS_urban_2AM_7200sec": (7200, 14400),
        "InTAS_highway_2AM_7200sec": (7200, 14400),
        "InTAS_urban_7AM_300sec": (25200, 25500),
        "InTAS_highway_7AM_300sec": (25200, 25500),
    }
    if set(manifest["scenarios"]) != set(expected_windows):
        raise PipelineError("Final scenario matrix changed unexpectedly")
    for name, scenario in manifest["scenarios"].items():
        start, end = scenario["communication_start"], scenario["communication_end"]
        if (start, end) != expected_windows[name]:
            raise PipelineError(f"{name} has an unexpected collection window")
    if int(manifest.get("pseudonym_interval_seconds", 0)) != 100:
        raise PipelineError("Final pseudonym interval must be 100 seconds")
    ...
    if [entry.get("type") for entry in manifest.get("detectors", [])] != [2, 6]:
        raise PipelineError("Final experiment evaluates exactly CAM Only and PRV")
    if manifest.get("analysis_datasets"):
        raise PipelineError("Final production runs must not include diagnostic datasets")
    expected_safety = {
        "minimum_available_disk_gib",
        "simulation_minimum_available_memory_gib",
        "attack_minimum_available_memory_gib",
        "evaluation_minimum_available_memory_gib",
    }
    if set(manifest.get("safety", {})) != expected_safety:
        raise PipelineError("Final host safety thresholds are incomplete")
    ...
    if dependency_check.returncode:
        raise PipelineError(
            ...
            "install Simulation-CPM-final/requirements.txt"
        )
...
def validate_prepared_scenario(
    ...
    expected_interval = f"{int(manifest['pseudonym_interval_seconds'])}s"
    if app["jsonPath"] != "/opt/mosaic/output/" or app["pseudonymInterval"] != expected_interval:
...
def validate_dataset(root: Path, start: int, end: int) -> dict[str, Any]:
    ...
    for kind in ("cam", "cpm", "ego"):
        ...
        for record in iter_records(directory):
            ...
            if "sendTime" in record:
                ...
                minimum_time = send_time if minimum_time is None else min(minimum_time, send_time)
                maximum_time = send_time if maximum_time is None else max(maximum_time, send_time)
        if not files or count == 0 or minimum_time is None or maximum_time is None:
            raise PipelineError(f"Empty or invalid {kind} dataset in {root}")
        if minimum_time < lower or maximum_time > upper:
        ...
        summary["min_send_time_ns"][kind] = minimum_time
        summary["max_send_time_ns"][kind] = maximum_time
    ...
    for sender, timeline in observations.items():
        ...
        if timeline:
            ...
            interval_ns = PSEUDONYM_INTERVAL_SECONDS * 1_000_000_000
            maximum_transitions = observed_span // interval_ns + 2
...
def run_container(
    container: dict[str, Any], scenario_mount: Path, runtime_name: str,
    output: Path, logs: Path, transcript: Path,
) -> None:
    command = [
        "docker", "run", "--rm", "--user", f"{os.getuid()}:{os.getgid()}",
        "--memory", str(container["memory_limit"]),
        "--memory-swap", str(container["memory_swap_limit"]),
        "--cpus", str(container["cpu_limit"]),
        ...
        container["image"], "-s", runtime_name,
        "-w", str(container["watchdog_seconds"]),
    ]
...
def run_simulation_job(manifest: dict[str, Any], setting: str, seed_id: str, jar: Path) -> Path:
    ...
    ensure_host_capacity(manifest, "simulation")
...
def run_attack_job(
    ...
    ensure_host_capacity(manifest, "attack")
    ...
    command = [
        ...
        "--attack-ratio", str(manifest["attack_ratio"]),
...
def evaluation_result_directory(dataset: Path, result_name: str) -> Path:
    return dataset.parent / "results" / dataset.name / result_name


def validate_evaluation_output(output: Path) -> None:
    predicted_path = output / "predicted.json"
    debug_path = output / "debug.json"
    if not predicted_path.is_file() or not debug_path.is_file():
        raise PipelineError(f"Incomplete detector output in {output}")
    with predicted_path.open(encoding="utf-8") as source:
        predicted = json.load(source)
    for key in ("tp", "tn", "fp", "fn", "f1", "false_positive_rate"):
        if key not in predicted:
            raise PipelineError(f"Detector output lacks {key}: {predicted_path}")
    if debug_path.stat().st_size < 2:
        raise PipelineError(f"Detector debug output is empty: {debug_path}")
    with debug_path.open("rb") as source:
        first = source.read(1)
        source.seek(-1, os.SEEK_END)
        last = source.read(1)
    if first != b"[" or last != b"]":
        raise PipelineError(f"Detector debug output is not a complete array: {debug_path}")


def run_evaluation_job(
    manifest: dict[str, Any], attack: str, attack_seed: int,
    simulation_seed: str, setting: str, detector: dict[str, Any],
) -> Path:
    dataset = attack_destination(attack, attack_seed, simulation_seed, setting)
    if not dataset.is_dir():
        raise PipelineError(f"Attacked dataset is missing: {dataset}")
    output = evaluation_result_directory(dataset, str(detector["result_name"]))
    if output.exists():
        try:
            validate_evaluation_output(output)
            print(
                f"[skip] completed detector output: {setting} / {attack} / "
                f"attack seed {attack_seed} / {detector['label']}"
            )
            return output
        except (PipelineError, OSError, ValueError, json.JSONDecodeError) as error:
            print(f"[discard] incomplete detector output {output}: {error}")
            shutil.rmtree(output)

    ensure_host_capacity(manifest, "evaluation")
    transcript = (
        LOG_ROOT / "evaluations" / setting / attack / f"attack-seed-{attack_seed}"
        / f"detector-{detector['type']}.log"
    )
    transcript.parent.mkdir(parents=True, exist_ok=True)
    command = [
        str(ATTACK_PYTHON), str(MBD_MAIN),
    ...
    print(
        f"[run] {setting} / {attack} / attack seed {attack_seed} / "
        f"{detector['label']}",
        flush=True,
    )
    with transcript.open("w", encoding="utf-8", buffering=1) as capture:
    ...
    if return_code:
        raise PipelineError(
            f"Detector exited with status {return_code}; see {transcript}"
        )
    validate_evaluation_output(output)
    print(
        f"[ok] {setting} / {attack} / attack seed {attack_seed} / "
        f"{detector['label']}"
    )
    return output
...
def safe_worker_count(
    stage: str, settings: list[str], requested: int | None, default: int,
) -> int:
    workers = default if requested is None else requested
    if workers < 1:
        raise PipelineError("Worker count must be at least one")
    maximum_by_stage = {
        "simulation": 1,
        "attack": 2,
        "evaluation": 4,
    ...
    try:
        maximum = maximum_by_stage[stage]
    except KeyError as error:
        raise PipelineError(f"Unknown parallel stage: {stage}") from error
    if workers > maximum:
        raise PipelineError(
            f"Refusing {stage} with {workers} workers: final host safety "
            f"allows at most {maximum} worker(s) for this stage"
        )
    return workers
...
def pipeline_command(manifest: dict[str, Any], jar: Path) -> None:
    """Run the published experiment sequentially, then create paper results."""
    settings = list(manifest["scenarios"])
    simulation_seeds = list(manifest["active_simulation_seeds"])
    attacks = list(manifest["attacks"])
    attack_seeds = list(manifest["active_attack_seeds"])

    run_parallel(
        run_simulation_job,
        [
            (manifest, setting, seed_id, jar)
            for seed_id in simulation_seeds for setting in settings
        ],
        1,
    )
    run_parallel(
    ...
    run_parallel(
        run_evaluation_job,
        [
            (manifest, attack, attack_seed, simulation_seed, setting, detector)
            for setting in settings for attack in attacks
            for attack_seed in attack_seeds for simulation_seed in simulation_seeds
            for detector in manifest["detectors"]
        ],
        1,
    )
    run_checked([
        str(ATTACK_PYTHON), str(RESULTS_GENERATOR), str(HERE),
        "--attack-seeds", *[str(seed) for seed in attack_seeds],
        "--simulation-seed", str(simulation_seeds[0]),
    ])
...
def parse_args() -> argparse.Namespace:
    ...
    subparsers.add_parser(
        "pipeline", help="Run simulation, three attack seeds, evaluation, and paper results"
    )
    subparsers.add_parser("validate", help="Build the app and statically validate all active scenarios")
    for command in ("simulate", "attack", "evaluate"):
        ...
        if command == "evaluate":
            ...
            item.add_argument("--attack-seeds", nargs="+", type=int)
        if command == "evaluate":
            item.add_argument("--detector-types", nargs="+", type=int)
...
def main() -> int:
    ...
    try:
        if args.command == "validate":
            ...
            return 0
        jar = validate_command(manifest)
        if args.command == "pipeline":
            pipeline_command(manifest, jar)
            return 0
        ...
        if args.command == "simulate":
            ...
            workers = safe_worker_count(
                "simulation", settings, args.jobs, int(manifest["simulation_jobs"])
            )
            run_parallel(
                run_simulation_job, jobs, workers,
            )
        if args.command == "attack":
            attacks = selected_values(args.attacks, manifest["attacks"], "attack")
            attack_seeds = args.attack_seeds or manifest["active_attack_seeds"]
            ...
            workers = safe_worker_count(
                "attack", settings, args.jobs, int(manifest["attack_jobs"])
            )
            run_parallel(
                run_attack_job, jobs, workers,
            )
        if args.command == "evaluate":
            ...
            if unknown_attack_seeds:
                raise PipelineError(f"Unknown attack seeds: {sorted(unknown_attack_seeds)}")
            available_detectors = {
                int(detector["type"]): detector for detector in manifest["detectors"]
            }
            detector_types = args.detector_types or list(available_detectors)
            unknown_detector_types = set(detector_types) - set(available_detectors)
            if unknown_detector_types:
                raise PipelineError(
                    f"Unknown detector types: {sorted(unknown_detector_types)}"
                )
            jobs = [
                (
                    manifest, attack, attack_seed, simulation_seed, setting,
                    available_detectors[detector_type],
                )
                for setting in settings for attack in attacks
                for attack_seed in attack_seeds for simulation_seed in simulation_seeds
            ...
            workers = safe_worker_count(
                "evaluation", settings, args.jobs,
                int(manifest["evaluation_jobs"]),
            )
            run_parallel(
                run_evaluation_job, jobs, workers,
            )
        return 0
