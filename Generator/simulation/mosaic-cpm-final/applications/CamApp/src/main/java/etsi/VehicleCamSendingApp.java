...
public class VehicleCamSendingApp extends AbstractCamSendingApp<VehicleOperatingSystem> {
    ...
    // Send at most one CPM per second from the same state snapshot as its CAM.
    ...
    private long lastCpmSendTime = Long.MIN_VALUE;
    ...
    private boolean communicationModuleEnabled = false;
    private boolean cpmPerceptionEnabled = false;
    ...
    /**
     * Enable the radio only when this vehicle is about to transmit inside the
     * configured communication window and area. Sampling, pseudonym changes,
     * and vehicle dynamics still begin at vehicle startup.
     */
    private void ensureCommunicationModuleEnabled() {
        if (communicationModuleEnabled) {
            return;
        }
        activateCommunicationModule();
        communicationModuleEnabled = true;
        getLog().infoSimTime(
                this,
                "Communication module enabled at first in-window, in-area CAM send"
        );
    }

    /**
     * Enable the CPM-only perception service at the first CAM send that is
     * already inside the configured communication time and area. CAM sampling
     * retains the original VeReMi lifecycle.
     */
    private void ensureCpmPerceptionEnabled() {
        if (cpmPerceptionEnabled) {
            return;
        }
        getOs().getPerceptionModule().enable(
                new SimplePerceptionConfiguration.Builder(VIEWING_ANGLE, VIEWING_RANGE)
                        .addModifiers(new WallOcclusion(), new BoundingBoxOcclusion())
                        .build()
        );
        cpmPerceptionEnabled = true;
        getLog().infoSimTime(
                this,
                "CPM perception enabled in communication window and area with wall occlusion, angle {} deg and range {} m",
                VIEWING_ANGLE,
                VIEWING_RANGE
        );
    }
    ...
    protected void sendCam() {
        ...
        ensureCommunicationModuleEnabled();
        ensureCpmPerceptionEnabled();
        super.sendCam();
        long currentTime = getOperatingSystem().getSimulationTime();
        if (lastCpmSendTime == Long.MIN_VALUE
                || currentTime - lastCpmSendTime >= CPM_INTERVAL) {
            sendCpm(data);
            lastCpmSendTime = currentTime;
        }
    }

    private void sendCpm(Data senderData) {
        JsonObject cpmJson = buildCpmJson(senderData);
        if (cpmJson == null) {
            return;
        }
    ...
    private void writeEgoCpmJson(JsonObject cpmJson) {
        try {
            ...
            getLog().error("Error while writing ego CPM json: {}", e.toString());
        }
    }
    ...
    private String getEgoCpmJsonPath() {
        ...
        if (!outputDirectory.exists() && !outputDirectory.mkdirs()) {
            getLog().error("Could not create ego JSON output directory: {}", outputDirectory.getAbsolutePath());
        }
    ...
    private JsonObject buildCpmJson(Data senderData) {
        if (senderData == null) {
        ...
        getLog().debugSimTime(this, "CPM perceived vehicles count: {}", perceivedVehicles.size());
        ...
        cpmJson.addProperty("messageID", "cpm_" + getOs().getId() + "_" + cpmMessageCounter++);
        ...
        cpmJson.add("perceivedObjects", createPerceivedObjectsJson(senderData.projectedPosition, perceivedVehicles));
    ...
    public void onMessageReceived(ReceivedV2xMessage receivedV2xMessage) {
        try {
            ...
            getLog().error("Error while parsing json: {}", e.toString());
        }
    }
    ...
    private String getReceivedJsonPath(ReceivedV2xMessage receivedV2xMessage) {
        ...
        if (!outputDirectory.exists() && !outputDirectory.mkdirs()) {
            getLog().error("Could not create JSON output directory: {}", outputDirectory.getAbsolutePath());
        }
